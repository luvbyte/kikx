...
const fs = new kikxSdk.FileSystemService();
...
const saveTextFile = async () => {
  ...
  const res = await fs.writeFile(currentFilePath, content);
...
const renderFiles = files => {
  ...
  files.forEach(file => {
    const wrapper = $("<div>").addClass("w-14 h-14 flex flex-col items-center");
